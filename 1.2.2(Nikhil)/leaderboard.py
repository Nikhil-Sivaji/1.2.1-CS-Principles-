...
def get_scores(file_name):
  ...
  for line in leaderboard_file:
    ...
    while(line[index] != ','):# to find the index of the comma
      index += 1
    index += 1## for skipping the comma and going to the number
    # TODO 4: use a while loop to get the score
    while(line[index] != '\n'):
      leader_score += line[index]
      print("current leader score :",leader_score,"index :",index)
      index += 1


    # TODO 5: add the player score to the scores list
   # converting the leader score into an int while adding it
